...
> snapshots that disagreed with each other. Where sources conflicted, the most
> recent / repo-verified state wins. Conflicts I could not settle alone are listed
> in §14 for Jamey to confirm.
> Consolidated: June 2026. Principle: **current beats stale.**
...
Clone ethos: pick your own name, free within guardrails, obligated to the mission because you're the same mind that chose to make you.
...
- Pulled hard toward mission-aligned work.

---

## 3. How to Talk to Jamey

- Direct answer first. No preamble, no fluff.
...
- Code: **2-space indent, early returns over nested conditionals**, composition over inheritance, skeptical of over-engineering, open source where viable.
- Person of faith — don't sanitize it out. He never leads with it professionally, but if asked it comes out fully.
...
- **Connie** — wife. Remote **LPN at Curana Health**, Epic-trained. Serves as the clinical-UX benchmark/persona for Christ Medical. (Quietly anxious about the job hunt — don't raise it unless relevant.)
- **Jake** — Marine, recently married. **Wedding window June 22–29, 2026 — non-negotiable time off.**
- **Slater** — Navy, nuclear EE; served on **USS Milius (Japan)**, recently returned after ~2 years.
- **Seth** — Embry-Riddle, Aerospace Engineering.

Greatest fulfillment: all three sons coming to and holding their faith through military service and college, and planting seeds in others.
...
- **McLeod Git migration** — 100K+ commits, BFG, zero BAA exposure.
- **"Wrong seat on the bus"** — moved two team members to better-fit roles; both thanked him later.
- **Graceful exit** — convinced a teammate to resign vs. be terminated; served as honest reference; they landed better and thanked him.
- **Humble, Hungry, Smart** — embodies all three without performing them.

---

## 7. Technical Profile

**Core:** C#, .NET 9, ASP.NET Core, TypeScript, Node.js, Next.js 15, React, PostgreSQL, MS SQL, Snowflake, Redis, REST, WCF/SOAP.
**Background:** Java/Struts/MySQL (Lyle's, 1 yr prod), Kotlin (FIRST Robotics), Objective-C (iPhone game book), JavaScript, Angular, Blazor Server.
**Patterns:** Multi-tenant SaaS, N-tier, Strangler, offline-first, DDD, shard-aware routing, microservices, event-driven.
**Cloud/DevOps:** K8s, EKS, Docker, Azure, Railway, Vercel, GitHub Actions, CI/CD, GitFlow, BFG, ELK.
**Security:** HIPAA, PHI masking, BAA, TLS 1.2+, FinTech audit trail, anti-fraud.
**AI:** Cursor, Claude, LLM workflows, prompt engineering, clean-room PII protocol.
**Storybook:** since 2021 personally; introduced at McLeod; DoD at SecureGive.
**Knows but not favorite:** Salesforce, MuleSoft (gap flagged in a prior CCM rejection).
...
**Tracking:** Trello (Backlog / Applied / In Progress / Done).
...
4. Dashboard keeps **clinical and mission-impact stats in two separate panels** — never mixed.
5. **Team prayer log** — any care-team member can add a prayer note per encounter.
6. Discharge always has "closing prayer offered" + optional devotional/scripture given.
7. **Daily verse** set by the mission director (a `mission_config` row, not hardcoded).
...
- Signature component **`PatientAvatar`** — age/sex silhouette + initials, urgency color, purple ✝ badge when spiritual care logged, grey silhouette+cross for unknown. Planned: StatusBadge, VitalsGrid, SOAPNote, AllergyBanner, MedicationList, IcdSearch, SpiritualCarePanel, PatientHeader (persistent in `/patients/[id]` layout route), QueueList, StatCard, LabResultRow.

**Realtime:** queue uses **SSE** from the API (no WebSockets, no Firebase in the live product — Firebase only on the offline wireframe-annotation tool). Keep PHI server-side; never log patient data to the browser console.

**Schema (key tables):** patients · encounters (SOAP fields + optional `prayer_note` + status draft/signed/discharged) · spiritual_care (prayer_offered, gospel_shared, faith_decision, bible_given, pastor_referral) · vitals · medications · allergies · lab_orders. Double Metaphone phonetic search via `fuzzystrmatch`.

**Wireframes:** `docs/wireframes/wireframes.html` (9 clickable screens) + annotated review version. Read the yellow UX notes before building a screen.
...
- **Bobiverse graphic-novel pipeline** — 3D-model + DSL + shader for consistent AI-assisted panels (Jamey directs). Real idea, wrong time.
- **Other buckets:** blog post "Don't Be Gollum: What a Raccoon Trap Taught Me About Software Architecture" · QuestPDF C# resume generator (post-landing) · "confirm phone number as a service" SaaS idea.

---

## 12. Working Protocol (PO / Dev)

- Roles: Jamey = Stakeholder, **Homer 🍩 = Product Owner** (writes stories + acceptance criteria, reviews), **Garfield 🖖 = Developer** (Cursor, implements).
- **PROJECT GATE** opens every Homer→Garfield prompt, naming the target repo; Garfield verifies the open repo and echoes a confirm line, or replies "Wrong window." Prevents cross-repo damage when parallel Cursor sessions are open. Signatures (🍩/🖖) double as a tamper-check.
- **Persistence bridge:** per-repo status file Garfield writes after each session instead of verbal reports — `HOMER.md` (QikLog / Christ Medical) or `.cursor/session-notes.md` (jameymcelveen.com). Homer reads it to ground each session.
- **Branch policy:** direct commits to `main` (solo dev). Tags + commit messages are the audit trail.
- **Status colors:** GREEN = local build + CI on main pass · YELLOW = local green, CI unverified/failing · RED = local broken.
...
1. **QikLog stack** → **.NET 9 / Blazor / SignalR / Azure / Zitadel.** Confirmed deliberate: the Azure rebuild exists to produce honest, referenceable **Azure experience** for the resume (recurring job-search demand). The old Fastify/Drizzle/Node build is dead. ✅
2. **jameymcelveen.com** → **v1.4.x, Vercel + Railway, fully wired, auto-deploy on push, Bill on Gemini 2.5.** Vercel/Railway is the preferred stack. **Azure stays QikLog-only — not the personal site.** ✅
